  ...
  <div id="content-main">
    <form action="{{ form_url }}" method="post" id="{{ opts.module_name }}_form" class="form-horizontal">
      ...
      <div>
        ...
        {% if form.errors %}
          <div class="alert alert-error">{% blocktrans count counter=form.errors.items|length %}Por favor corrija el siguiente error.{% plural %} Por favor corrija los siguientes errores.{% endblocktrans %}</div>
        {% endif %}
